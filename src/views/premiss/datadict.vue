  ...
  <div>
    <div class="app-container calendar-list-container" v-show="addDialog" >
      <div class="filter-container">
        ...
        <el-button class="filter-item" type="primary" v-waves icon="el-icon-search" @click="handleFilter">{{$t('premiss.filter.search')}}</el-button>
      ...
      <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border stripe fit highlight-current-row
        ...
        <el-table-column align="center" :label="$t('premiss.table.actions')" min-width="180" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" icon="el-icon-edit" @click="add">{{$t('premiss.table.edit')}}</el-button>
    ...
    <div v-show="!addDialog">
      <add-dict v-bind:addDialog="addDialog" v-on:c="getback($event)"></add-dict>
    </div>
  </div>
...
import adddict from './add/adddict'
